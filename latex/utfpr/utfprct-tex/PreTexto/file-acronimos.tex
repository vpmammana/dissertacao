...
%% Este arquivo para defini\c{c}\~ao de abreviaturas, siglas e acr\^onimos \'e utilizado com a op\c{c}\~ao \incluirlistadeacronimos{file} 
%%
%% Vantagens do modo com "file" em rela\c{c}\~ao ao modo "glossaries":
%% 	1) Inser\c{c}\~ao tabular da listas
%% 	2) Controle da ordem de apresenta\c{c}\~ao das listas
%% 	3) N\~ao \'e preciso referenciar no texto

\listadeabrevsiglaseacr%% Formata\c{c}\~ao da lista de abreviaturas, siglas e acr\^onimos


\begin{listadesiglas}%% Ambiente listadesiglas (Siglas: Representa\c{c}\~ao & Defini\c{c}\~ao \\)
