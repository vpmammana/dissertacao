...
\'E neste ponto do texto que alcan\c{c}amos o final do caminho percorrido pelos m\'etodos descritos no cap\'{\i}tulo anterior, criando as condi\c{c}\~oes para que os resultados obtidos sejam apresentados.
...
Os resultados obtidos ser\~ao descritos e, concomitantemente, analisados para, posteriormente, discut\'{\i}-los.
...
Os resultados ser\~ao, inicialmente, apresentados separadamente, em tr\^es se\c{c}\~oes relacionadas \`as tr\^es dimens\~oes em an\'alise neste texto:
...
Importante registrar que os dados a seguir foram obtidos a partir da contribui\c{c}\~ao de v\'arios colaboradores do WASH, com nossa participa\c{c}\~ao ativa na especifica\c{c}\~ao dos sistemas de coleta de dados, a exemplo da PLataforma Platu\'osh, da qual a presente candidata \'e co-autora.
...
\begin{alineas}
\item Plataforma Platu\'osh, da qual esta candidata \'e co-autora
...
\subsection[Caracteriza\c{c}\~ao do p\'ublico atendido]{Caracteriza\c{c}\~ao do p\'ublico atendido}\label{Caracteriza\c{c}\~ao do p\'ublico atendido}
Para caracterizar o p\'ublico atendido pelo WASH ser\~ao explorados os seguintes indicadores:
...
\begin{alineas}
\item N\'umero total de pessoas atendidas
\item Evolu\c{c}\~ao temporal do n\'umero de pessoas atendidas
\item Distribui\c{c}\~ao de participantes por sexo
\end{alineas}

Antes de prosseguir \'e preciso explicitar que os indicadores acima foram obtidos por amostragem, uma vez que o Projeto WASH tem limita\c{c}\~oes na coleta de dados cadastrais de participantes.
...
Portanto, \'e comum n\~ao haver registro no WASH dos participantes que est\~ao sob cust\'odia da Entidade Respons\'avel.
...
Inclusive, a partir da edi\c{c}\~ao da Lei Geral de Prote\c{c}\~ao de Dados [XXX], foi poss\'{\i}vel identificar uma maior resist\^encia, por parte dos parceiros, em compartilhar informa\c{c}\~oes de participa\c{c}\~ao dos estudantes, o que \'e perfeitamente compreens\'{\i}vel.
...
Assim, no que tange \`a caracteriza\c{c}\~ao do p\'ublico participantes, \'e preciso usar o m\'etodo de amostragem.
...
Antecipadamente, para evitar que o leitor tenha uma vis\~ao inicial equivocada sobre a representatividade estat\'{\i}stica dos dados aqui apresentado \'e preciso fazer uma ressalva, \`a qual retornaremos in\'umeras vezes neste texto.
...
No que se refere \`a distribui\c{c}\~ao de participantes por sexo, foi aplicado um m\'etodo de amostragem especial, principalmente para os casos em que o cadastro era incompleto e n\~ao havia informa\c{c}\~ao individualizada sobre sexo. O Cap\'{\i}tulo de Materias e M\'etodos detalha como esta identifica\c{c}\~ao \`a posteriore de sexo foi feita mas, sumariamente, podemos dizer que ela se deu por meio da identifica\c{c}\~ao do g\^enero do primeiro nome de cada particante, por meio da compara\c{c}\~ao com listas de nomes extensivas.
...
O emprego de uma consulta SQL na base indica a exist\^encia de 3265 participantes com cadastros conhecidos. Este n\'umero \'e uma amostra dos participantes, como j\'a explicado.
...
Para obter este n\'umero de participantes foi tomado o cuidado de n\~ao contabilizar nomes repetidos, uma vez que o tipo de consulta SQL realizada foi concebida para n\~ao contar ocorr\^encias repetidades de participantes (participantes com o mesmo nome). Sem esse cuidado de excluir os nomes repetidos, a base de dados contabiliza um total de 3312 participantes.
